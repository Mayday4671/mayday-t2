    ...
    <section class="hero-section">
      <div class="hero-content">
        <h1 class="hero-title" ref="heroTitle">
          连接未来的 <br />
          <span class="highlight">分布式智能体网络</span>
        </h1>
        <p class="hero-subtitle" ref="heroSubtitle">
          Mayday 是全球首个去中心化 AI 算力与推理平台。我们将数万亿智能终端连接成一个巨大的行星级超级大脑。
        </p>
        <div class="cta-group" ref="heroCta">
          <a-button type="primary" size="large" class="cta-btn primary">立即开始</a-button>
          <a-button type="default" size="large" class="cta-btn ghost">阅读白皮书</a-button>
        </div>
      </div>
    </section>
...
const animate = () => {
  ...
  if (particleSystem) {
    ...
    particleSystem.rotation.y += 0.00001 + (mouseX * 0.000005); 
...
.highlight {
  background: linear-gradient(90deg, #00c6fb 0%, #005bea 50%, #00c6fb 100%);
  background-size: 200% auto;
  background-clip: text;
  -webkit-background-clip: text;
  -webkit-text-fill-color: transparent;
  animation: shine 3s linear infinite;
  text-shadow: 0 0 30px rgba(0, 198, 251, 0.5);
}
